...
Service: ${payload.new?.service||"?"}`, icon:"/logo.gif"}); } catch(_){}
  ...
  const shareOfferte = async (offerte) => {
    try {
      ...
      // Capture doc HTML (klein, zonder CSS) voor offerte.html weergave
      let _docHtml = null;
      try {
        const dw = document.querySelector(".mb-body .doc-wrap");
        if(dw) _docHtml = dw.outerHTML;
      } catch(e) { console.warn("[Share] HTML capture:", e.message); }
      ...
      console.log(`[Share] Payload: ${kb}KB`);
      if(kb>850){delete shareData._docHtml;console.warn("[Share] docHtml weggooien, te groot");}
      if(Math.round(JSON.stringify(payload).length/1024)>850){shareData.lijnen=shareData.lijnen.map(l=>({...l,technischeFiches:null}));console.warn("[Share] Fiches weggooien");}
      const{error:uErr}=await sb.from("offerte_shares").upsert(payload,{onConflict:"id"});
      if(uErr){console.error("[Share] FAILED:",uErr.message);delete shareData._docHtml;shareData.lijnen=shareData.lijnen.map(l=>({...l,technischeFiches:null}));const{error:r2}=await sb.from("offerte_shares").upsert(payload,{onConflict:"id"});if(r2)throw new Error(r2.message);}
...
<div class="wrap">
  ...
  const verzend = async () => {
    ...
    try {
      ...
      if(pubKey && serviceId && templateId && window.emailjs) {
        await window.emailjs.send(serviceId, templateId, {
        ...
        const blob = new Blob([html],{type:"text/html"});
        const a = document.createElement("a");
        a.href = URL.createObjectURL(blob); a.download = `aanmaning_${f.nummer}.html`; a.click();
        setTimeout(()=>URL.revokeObjectURL(a.href),5000);
    ...
    } catch(e) { alert("Fout bij verzenden: "+e.message); }
    setSending(false);
  };
    ...
    <div className="mo"><div className="mdl" style={{maxWidth:700}}>
      ...
      <div className="mb-body" style={{padding:20}}>
        <div style={{marginBottom:18}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {templates.map((tt,i)=>(
              <button key={i} onClick={()=>setNiveau(i+1)} style={{padding:"10px 16px",borderRadius:10,border:`2px solid ${niveau===i+1?tt.c:"#e2e8f0"}`,background:niveau===i+1?tt.c+"22":"#fff",color:tt.c,fontWeight:700,fontSize:13,cursor:"pointer"}}>{tt.l}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{marginBottom:14}}>
          <button onClick={()=>setPreview(p=>!p)} style={{fontSize:12,background:"#f1f5f9",border:"1px solid #e2e8f0",borderRadius:6,padding:"6px 12px",cursor:"pointer"}}>{preview?"▲ Verberg preview":"▼ Toon email preview"}</button>
        </div>
        {preview&&<div style={{border:"1px solid #e2e8f0",borderRadius:10,overflow:"hidden",marginBottom:14}}>
          <iframe srcDoc={buildHtmlEmail()} style={{width:"100%",height:380,border:"none"}} title="Email preview"/>
        </div>}
    ...
    <div style={{padding:"0 0 40px"}}>
      ...
      {gefilterd.length===0
        ...
        : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:12}}>
          {gefilterd.map(lead=>{const lFotos=getFotos(lead);return(
            <div key={lead.id} onClick={()=>{setSelected(lead);setFotoIdx(null);}} style={{background:"#fff",border:`2px solid ${selected?.id===lead.id?"#3b82f6":"#e2e8f0"}`,borderRadius:14,padding:"16px",cursor:"pointer",transition:"all .15s",boxShadow:selected?.id===lead.id?"0 0 0 3px #bfdbfe":"0 1px 4px rgba(0,0,0,.06)"}}>
              ...
              {lFotos.length>0&&<div style={{marginTop:10,display:"flex",gap:4}}>{lFotos.slice(0,5).map((f,i)=>{const src=getFotoSrc(f);return src?<img key={i} src={src} alt="" style={{width:40,height:40,objectFit:"cover",borderRadius:6,border:"1px solid #e2e8f0",flexShrink:0}} onError={e=>{e.target.style.display="none";}}/>:null;})}{lFotos.length>5&&<div style={{minWidth:40,height:40,borderRadius:6,background:"#f0f4f8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#64748b"}}>+{lFotos.length-5}</div>}</div>}
            </div>
          );})}
        </div>
      }
                        ...
                        {fotos.map((foto,i)=>{const src=getFotoSrc(foto);const label=getFotoLabel(foto,i);return(
                          <div key={i} onClick={()=>setFotoIdx(i)} style={{cursor:"zoom-in"}}>
                            <div style={{borderRadius:10,overflow:"hidden",border:"2px solid #e2e8f0",aspectRatio:"1",background:"#e8edf2"}}>
                              {src?<img src={src} alt={label} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} onError={e=>{e.target.style.display="none";}}/>:<div style={{display:"flex",height:"100%",alignItems:"center",justifyContent:"center",color:"#94a3b8",fontSize:11,padding:4}}>Geen URL</div>}
                            </div>
